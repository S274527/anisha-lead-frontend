...
export const EditForm = () => {
    ...
    if (lead && lead?.id) {
      reset({
        ...
        contact_number: lead?.contact_number
          ? `${lead?.phone_code}${lead?.contact_number}`
          : "",
        status: lead?.status ?? "",
        source: lead?.source ?? "",
        ...
        // @ts-ignore
        follow_up_date: dayjs(lead?.follow_up_date).format('DD-MM-YYYY')
      });
      setStartDate(lead?.follow_up_date as any);
    }
  }, [lead]);
  ...
  const { mutate: editLead } = useMutation(editLeadRequest, {
    onSuccess: (res) => {
  ...
  const onSubmit = (data: any) => {
    ...
    let payload = {
      ...
      user_id: session?.user?.type === 'user' ? session?.user?.id?.toString() : data?.user_id?.toString()
    };
    editLead({
      id: lead?.id,
      ...payload,
                  ...
                  render={({
                    ...
                    <InputSelect
                      ...
                      value={userOptions.find(
                        (item: any) => item.id === value?.toString() || item.id === lead?.user_id
                      )}
                      label="User"
